        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <select
                value={filters.state || ""}
                onChange={(e) => setFilters({ ...filters, state: e.target.value, district: "", location: "" })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFilters({ ...filters, district: e.target.value, location: "" })}
        ...
        <div className="space-y-6">
          {filteredJobs.length > 0 ? filteredJobs.map(job => (
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-gray-600 font-medium mb-1">{job.user?.name || job.employer}</p>
                      <p className="text-red-600 mb-2 flex items-center"><span className="mr-2">📍</span>{job.location}</p>
                    </div>
                    <div className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-medium">{job.category}</div>
                  </div>
                  <p className="text-gray-600 mb-4 leading-relaxed">{job.description}</p>
                  <div className="flex flex-wrap gap-4 mb-4">
                    <div className="bg-green-50 text-green-700 px-3 py-2 rounded-lg">
                      <span className="font-semibold">₹{job.salaryMin} - ₹{job.salaryMax}</span>
                  ...
                  {job.requirements && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                      <p className="text-yellow-800 text-sm"><strong>Requirements:</strong> {job.requirements}</p>
                    </div>
                  )}
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
              ...
              <button onClick={clearFilters} className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-6 rounded-lg transition-colors duration-200">
                Clear Filters
              </button>
            </div>
          )}
